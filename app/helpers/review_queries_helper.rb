...
module ReviewQueriesHelper
  ...
  def column_content(column, review)
    value = column.value_object(review)
    if value.is_a?(Array)
      value.collect {|v| column_value(column, review, v)}.compact.join(', ').html_safe
    else
      column_value(column, review, value)
    end
  end
  ...
  def csv_content(column, review)
    value = column.value_object(review)
    if value.is_a?(Array)
      value.collect {|v| csv_value(column, review, v)}.compact.join(', ')
    else
      csv_value(column, review, value)
    end
  end
  ...
  def csv_value(column, object, value)
    format_object(value, false) do |value|
      case value.class.name
        ...
        when 'ReviewRelation'
          other = value.other_review(object)
  ...
  def retrieve_query
    if !params[:query_id].blank?
      ...
      @query = ReviewQuery.where(cond).find(params[:query_id])
